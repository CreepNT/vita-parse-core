...
def main():
    ...
    parser.add_argument("-m", "--dump-module-info", dest="dumpmodule",
                        action="store_true", help="Enable dumping of the modules' info.", default=False)
    parser.add_argument("-A", "--dump-all-threads", dest="dumpall",
                        action="store_true", help="Enables dumping of all threads' infos, even the non-crashed ones.", default=False)
    ...
    if(args.dumpall):
        ...
        with indent():
            for thread in core.threads:
                if not (thread in crashed):
                    non_crashed.append(thread)
        for thread in non_crashed:
            iprint('=== THREAD "{}" <0x{:x}> INFO ==='.format(thread.name, thread.uid))
